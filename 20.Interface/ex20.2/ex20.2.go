...
// 메서드가 응집도(cohesiveness)를 높이기 위한 것이라면 인터페이스는 의존성를 낮추려는 목적(decoupling)
// 내부 동작을 감춰서 서비스 제공자와 서비스 사용자 모두에게 자유를 주는 방식 (추상화) - 서비스 제공자는 안에 어떻게 구현을 하든 Send라는 메서드만 있으면 되고 서비스 사용자는 안에 어떤지 관심없고 Send라는 메서드만 쓰면 됨
